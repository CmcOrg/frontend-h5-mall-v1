...
import {
    TakeawaySpuDeleteByIdSet,
    TakeawaySpuDO,
    TakeawaySpuInfoById,
    TakeawaySpuInsertOrUpdate,
    TakeawaySpuInsertOrUpdateDTO,
    TakeawaySpuPage,
    TakeawaySpuPageDTO
...
export default function () {
    ...
    const [useForm] = Form.useForm<TakeawaySpuInsertOrUpdateDTO>();

    const [formVisible, setFormVisible] = useState<boolean>(false);

    const currentForm = useRef<TakeawaySpuInsertOrUpdateDTO>({} as TakeawaySpuInsertOrUpdateDTO)

    return (
        <>
            <ProTable<TakeawaySpuDO, TakeawaySpuPageDTO>
                ...
                request={(params, sort, filter) => {
                    return TakeawaySpuPage({...params, sort})
                }}
                toolbar={{
                    actions: [
                        <Button key={"1"} icon={<PlusOutlined/>} type="primary" onClick={() => {
                            currentForm.current = {} as TakeawaySpuInsertOrUpdateDTO
                            setFormVisible(true)
                        }}>新建</Button>
                    ],
                }}
                tableAlertOptionRender={({selectedRowKeys, selectedRows, onCleanSelected}) => (
                    <Space size={16}>
                        <a className={"red3"} onClick={() => {
                            ExecConfirm(() => {
                                return TakeawaySpuDeleteByIdSet({idSet: selectedRowKeys as number[]}).then(res => {
            ...
            <BetaSchemaForm<TakeawaySpuInsertOrUpdateDTO>
                    ...
                    render: (props, dom) => {
                        return [
                            ...
                            currentForm.current.id ? <Button
                                ...
                                onClick={() => {
                                    ExecConfirm(async () => {
                                        return TakeawaySpuDeleteByIdSet({idSet: [currentForm.current.id!]}).then(res => {
                ...
                request={async () => {
                    ...
                    if (currentForm.current.id) {
                        await TakeawaySpuInfoById({id: currentForm.current.id}).then(res => {
                            currentForm.current = res as TakeawaySpuInsertOrUpdateDTO
                        })
                    }
