...
import {
    ...
    TakeawaySkuDO,
    TakeawaySkuInfoById,
    TakeawaySkuInsertOrUpdate,
    TakeawaySkuInsertOrUpdateDTO,
    TakeawaySkuPage,
    TakeawaySkuPageDTO
} from "@/api/admin/TakeawaySkuController";
...
export default function () {
    ...
    return (
        <>
            <ProTable<TakeawaySkuDO, TakeawaySkuPageDTO>
                ...
                request={(params, sort, filter) => {
                    return TakeawaySkuPage({...params, sort})
                }}
                toolbar={{
                    actions: [
                ...
                tableAlertOptionRender={({selectedRowKeys, selectedRows, onCleanSelected}) => (
                    <Space size={16}>
                        <a className={"red3"} onClick={() => {
                            ExecConfirm(() => {
                                return TakeawaySkuDeleteByIdSet({idSet: selectedRowKeys as number[]}).then(res => {
            ...
            <BetaSchemaForm<TakeawaySkuInsertOrUpdateDTO>
                title={currentForm.current.id ? "编辑商品SKU" : "新建商品SKU"}
                    ...
                    render: (props, dom) => {
                        return [
                            ...
                            currentForm.current.id ? <Button
                                ...
                                onClick={() => {
                                    ExecConfirm(async () => {
                                        return TakeawaySkuDeleteByIdSet({idSet: [currentForm.current.id!]}).then(res => {
                ...
                request={async () => {
                    ...
                    if (currentForm.current.id) {
                        await TakeawaySkuInfoById({id: currentForm.current.id}).then(res => {
                            currentForm.current = res as TakeawaySkuInsertOrUpdateDTO
                        })
                    }
